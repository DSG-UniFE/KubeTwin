...
# KOptimizerMultiobjective#encode_replicas_set / #decode_cluster_mapping
# delegate to KUBETWIN::VectorCodec (see vector_codec_spec.rb for the
# exhaustive behavior coverage; this spec only checks the wiring). Built
# via .allocate with just the ivars these two methods need, rather than
# through #initialize, which loads a real simulation Configuration and
# writes a log file.
describe KUBETWIN::KOptimizerMultiobjective do
  def build_optimizer(rss:, n_ms:, max_replicas: 10)
    optimizer = KUBETWIN::KOptimizerMultiobjective.allocate
