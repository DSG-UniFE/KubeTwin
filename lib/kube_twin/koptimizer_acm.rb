...
module KUBETWIN
  class KOptimizerACM
    ...
    def initialize(configuration_file)
      ...
      @logger.info @sim_conf.replica_sets.to_s
    ...
    #
    # NOTE: this used to build replicas_per_ms by mutating its `rss`
    # argument in place instead of a deep copy (unlike every other
    # KOptimizer* class's version of this method) -- an apparent copy-paste
    # drift from koptimizer.rb, not an intentional difference. Delegating
    # to VectorCodec fixes that: rss is now deep-copied here too, so
    # @sim_conf.replica_sets is never mutated as a side effect of a PSO
    # evaluation.
