...
class ChaosEnvDeepSet(gym.Env):
    ...
    def _connect_to_socket(self, env_id=1):
        ...
        while i < max_attempts:
            try:
                ...
                logging.error("Error in connecting to UNIX socket, retrying in 0.5 seconds...", server_address)
                time.sleep(0.5)
            else:
                break
        if i == max_attempts:
    ...
    def reallocate_pod(self, node_id, pod_id):
        ...
        try:
            ...
            logging.error("Error in sending data to simulator for the pod reallocation through UNIX socket: {e}")
            exit(1)
